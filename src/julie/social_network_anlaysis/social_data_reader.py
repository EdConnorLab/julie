...
def main():
    ...
    new_rows = []
    for index, row in agonistic.iterrows():
        if ',' in row['Social Modifier']:
            modifiers = row['Social Modifier'].split(',')
            for modifier in modifiers:
                new_rows.append({col: row[col] for col in agonistic.columns})
                new_rows[-1]['Social Modifier'] = modifier
        else:
            new_rows.append({col: row[col] for col in agonistic.columns})
    new_df = pd.DataFrame(new_rows)

    edge_weights = new_df.groupby(['Focal Name', 'Social Modifier']).size().reset_index(name='weight')
    print(edge_weights)
    ...
    nx.draw(G, pos, with_labels=True, node_size=700, node_color="skyblue", font_size=12, arrowsize=20, width=weights,
            edge_color='darkblue')
    plt.show()
